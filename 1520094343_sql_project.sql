...
SELECT COUNT(membercost) AS count_of_free_membership
FROM `Facilities`
WHERE membercost = 0.0;
...
SELECT surname, firstname, joindate
 FROM `Members` 
WHERE joindate = (SELECT MAX(joindate) AS most_recent
FROM Members)
...
WHERE  Bookings.starttime BETWEEN  '2012-09-14' AND '2012-09-15' AND (
...
SELECT *  FROM (
        SELECT Facilities.name , SUM(CASE WHEN Bookings.memid = 0 THEN Facilities.guestcost*Bookings.slots  
